...
function mostrar_basico( $atts ) {
	...
	if ($user_login)
		return '<div>
    <p><b>'.esc_html__("Active!", 'nodechartsfam').'</b></p>
    </div>';
	else{
		$my_current_lang = apply_filters( 'wpml_current_language', NULL );
        if ($my_current_lang == "es"){
			return '<a class="link-pricing" href="https://nodecharts.com/registrarse"><div class="boton"><p>'.esc_html__("Start free", 'nodechartsfam').'</p></div></a>';
		}else{
			return '<a class="link-pricing" href="https://nodecharts.com/en/sign-up"><div class="boton"><p>'.esc_html__("Start free", 'nodechartsfam').'</p></div></a>';
		}
	}
}
...
function mostrar_principiante_anual( $atts ) {
	...
	if ($user_login){
		if ( is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ) {
			...
			if ($subscription == "Principiante Anual" || $subscription == "Experto Anual" || $subscription == "Profesional Anual"){
			return '<div>
			<p><b>'.esc_html__("Active!", 'nodechartsfam').'</b></p>
			</div>';
			}else{
			return '<a rel="nofollow" href="?add-to-cart=358" data-quantity="1" data-product_id="358" data-product_sku="principiante-anual" class="button product_type_simple add_to_cart_button ajax_add_to_cart added"><div class="boton"><p>'.esc_html__("Buy!", 'nodechartsfam').'</p></div></a>';
			}
		}else{
			return '<p style="color:red;">Woocommerce Subscription no encontrado</p>';
		} 
...
function mostrar_experto_mensual( $atts ) {
	...
	if ($user_login){
		if ( is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ) {
			...
			if ($subscription == "Experto Mensual" || $subscription == "Experto Anual" || $subscription == "Profesional Anual"){
			return '<div>
			<p><b>'.esc_html__("Active!", 'nodechartsfam').'</b></p>
			</div>';
			}else{
...
function mostrar_experto_anual( $atts ) {
	...
	if ($user_login){
		if ( is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ) {
			...
			return '<a rel="nofollow" href="?add-to-cart=361" data-quantity="1" data-product_id="361" data-product_sku="experto-anual" class="button product_type_simple add_to_cart_button ajax_add_to_cart added"><div class="boton"><p>'.esc_html__("Buy!", 'nodechartsfam').'</p></div></a>';
			}
		}else{
			return '<p style="color:red;">Woocommerce Subscription no encontrado</p>';
		} 	
...
function mostrar_profesional_mensual( $atts ) {
	...
	if ($user_login){
		if ( is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ) {
			...
			return '<div class="boton disable">
			<p style="color: gray!important;">'.esc_html__("Buy!", 'nodechartsfam').'</p>
			</div>';
			}
		}else{
			return '<p style="color:red;">Woocommerce Subscription no encontrado</p>';
